/*
  This node subscribes to a topic which publishes a point cloud, then performs 
  Euclidean Cluster Extraction and published the clusters as a new point cloud
*/
#include "ros/ros.h"
...
{ 
  ...
  // Concatenating each of the clusters extracted into a single point cloud
  ...
  for (const auto& cluster : cluster_indices)
  {
    ...
    {
      pcl::concatenatePointCloud(msg_final,msg,msg_final);
    }

    j++;
  }
  cluster_publisher.publish(msg_final);
}
...
int main(int argc, char **argv)
{
  ...
  cluster_publisher = n.advertise<PointCloud>("/euclid_clusters", 1000);
  ros::spin();
