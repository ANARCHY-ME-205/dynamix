...
def drive_rgb (img : Image):
    ...
    if not publishing_started:
        ...
        sub_depth = Subscriber("/zed2i/zed_node/depth/depth_registered", Image)
        sub_camera_info = Subscriber("/zed2i/zed_node/depth/camera_info", CameraInfo)

        # Create the ApproximateTimeSynchronizer
        ats = ApproximateTimeSynchronizer([sub_depth, sub_camera_info], queue_size=10, slop=0.1)
        ats.registerCallback(drive_depth)
    # cv2.waitKey(1)

def drive_depth(img: Image, camera_info: CameraInfo):
...
if __name__ == '__main__':
    ...
    pub4 = rospy.Publisher("/zed2i/camera_info", CameraInfo, queue_size=10)
    # Create the subscribers for RGB image and depth image
    sub_rgb = Subscriber("/zed2i/zed_node/rgb/image_rect_color", Image)
    
    

    # Subscribe to the RGB image topic
    sub_rgb = rospy.Subscriber("/zed2i/zed_node/rgb/image_rect_color", Image, callback=drive_rgb)
